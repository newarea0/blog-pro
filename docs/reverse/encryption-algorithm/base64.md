...
var Base64 = {
  ...
  // 将输入字符串进行Base64编码的方法
  encode(input) {
    let output = '' // 最终的Base64编码结果
    let chr1, chr2, chr3 // 三个字符的ASCII码
    let enc1, enc2, enc3, enc4 // 四个6位Base64字符的索引
    let i = 0
  ...
  decode(input) {
    let output = '' // 最终的解码结果
    let chr1, chr2, chr3 // 三个字符的ASCII码
    let enc1, enc2, enc3, enc4 // 四个Base64字符的索引值
    let i = 0
...
// 对字符串进行Base64编码
const resultEncoded = Base64.encode(stringChinese)

// 对Base64编码的字符串进行解码
const resultDecoded = Base64.decode(resultEncoded)
...
// 示例
const plainText = 'https://studypy.blog.csdn.net'
const encodedText = customBase64Encode(plainText)
const decodedText = customBase64Decode(encodedText)
